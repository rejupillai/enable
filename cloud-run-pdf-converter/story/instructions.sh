...
gcloud config set project neivtron 
GOOGLE_CLOUD_PROJECT=neivtron
PROJECT_NUMBER=484316071929


gcloud beta run deploy cloud-run-pdf-converter \
  --image gcr.io/$GOOGLE_CLOUD_PROJECT/cloud-run-pdf-converter \
  --platform managed \
  --region us-central1 \
  --memory=2Gi \
...
SERVICE_URL=https://cloud-run-pdf-converter-ciu52aqa7q-uc.a.run.app
...
gcloud beta run services add-iam-policy-binding cloud-run-pdf-converter --member=serviceAccount:pubsub-cloud-run-invoker@$GOOGLE_CLOUD_PROJECT.iam.gserviceaccount.com --role=roles/run.invoker --platform managed --region us-central1
...
# Review the output
gsutil -m list gs://$GOOGLE_CLOUD_PROJECT-processed
